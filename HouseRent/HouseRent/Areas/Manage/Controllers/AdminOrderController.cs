...
namespace HouseRent.Areas.Manage.Controllers
{
    ...
    public class AdminOrderController : Controller
    {
        ...
        {
            var query = _appDbContext.Orders.Include(x => x.OrderItems).Include(x => x.Apartment.ApartmentCategory).Include(x => x.Apartment.ApartmentFeatures).Include(x => x.Apartment.ApartmentImages).Where(x => x.OrderStatus == Enum.OrderStatus.Accepted).AsQueryable();
            PaginatedList<Order> orders = PaginatedList<Order>.Create(query, 4, page);
            return View(orders);
        }
